...
var Sentry = (function(exports) {
  ...
  function addExceptionTypeValue(event, value, type, mechanism) {
    ...
    event.exception.values[0].mechanism =
      event.exception.values[0].mechanism || mechanism;
  }
    ...
    Scope.prototype.addBreadcrumb = function(breadcrumb, maxBreadcrumbs) {
      ...
      this._breadcrumbs =
        maxBreadcrumbs !== undefined && maxBreadcrumbs >= 0
          ? __spread(this._breadcrumbs, [normalize(mergedBreadcrumb)]).slice(
              -maxBreadcrumbs
            )
  ...
  var Hub = /** @class */ (function() {
    ...
    Hub.prototype.captureException = function(exception, hint) {
      ...
      this._invokeClient(
        'captureException',
        exception,
        __assign({}, hint, { event_id: eventId })
      );
      return eventId;
    };
    /**
  ...
  var BaseClient = /** @class */ (function() {
     ...
     * @inheritDoc
     */
    BaseClient.prototype.captureMessage = function(
      message,
      level,
      hint,
      scope
     ...
     */
    BaseClient.prototype._handleAsyncBeforeSend = function(
      beforeSend,
      resolve,
      reject
    ) {
    ...
    function _computeStackTraceFromOperaMultiLineMessage(ex) {
      ...
      var lineRE1 = /^\s*Line (\d+) of linked script ((?:file|https?|blob)\S+)(?:: in function (\S+))?\s*$/i,
        ...
        scripts =
          window$1 &&
          window$1.document &&
          window$1.document.getElementsByTagName('script'),
        inlineScriptBlocks = [],
        parts;
  ...
  var _installGlobalUnhandledRejectionHandler =
  ...
  function eventFromPlainObject(exception, syntheticException) {
    ...
    var event = {
      ...
      message:
        ...
        keysToEventMessage(exceptionKeys),
    };
  ...
  var BrowserBackend = /** @class */ (function(_super) {
    ...
    BrowserBackend.prototype.eventFromException = function(exception, hint) {
      ...
      if (isDOMError(exception) || isDOMException(exception)) {
        ...
        var message_1 = domException.message
  ...
  function breadcrumbEventHandler(eventName, debounce) {
    ...
    return function(event) {
      ...
      var captureBreadcrumb = function() {
        ...
        try {
          target = event.target
            ? _htmlTreeAsString(event.target)
            : _htmlTreeAsString(event);
        } catch (e) {
          target = '<unknown>';
        }
        if (target.length === 0) {
  ...
  var GlobalHandlers = /** @class */ (function() {
    ...
    GlobalHandlers.prototype._eventFromGlobalHandler = function(stacktrace) {
      ...
      var fallbackValue = stacktrace.original
        ? truncate(
            JSON.stringify(normalize(stacktrace.original)),
            maxValueLength
          )
        : '';
      var fallbackType =
  ...
  var Breadcrumbs = /** @class */ (function() {
          ...
          return function() {
            ...
            if (level === 'assert') {
              if (args[0] === false) {
                ...
                breadcrumbData.data.extra.arguments = normalize(
                  args.slice(1),
                  3
                );
              }
    ...
    Breadcrumbs.prototype._instrumentDOM = function() {
      ...
      global$3.document.addEventListener(
        'click',
